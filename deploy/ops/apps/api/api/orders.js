  ...
  const fromBody =
    typeof payload?.siteCode === "string"
      ...
      : typeof payload?.site_code === "string"
        ...
        : "";
  const siteCode =
    (fromCtx && fromCtx !== "default" ? fromCtx : "") || fromEnv || fromBody || "default";
  return { ...payload, siteCode };
}

async function placeOrderPostgres(payload) {
  const merged = mergeSiteCodeIntoPayload(payload);
  const siteForRpc = getSiteCode() !== "default" ? getSiteCode() : getEnv("SITE_CODE") || null;

  let data;
  try {
    data = await placeOrderRpc(merged, siteForRpc || null);
  } catch (rpcErr) {
    const mapped = mapSupabaseOrderError(rpcErr instanceof Error ? rpcErr.message : String(rpcErr));
  ...
  if (!data || data.ok !== true) {
    const mapped = mapSupabaseOrderError(data?.error || "建立訂單失敗");
    const err = new Error(mapped.error);
    err.status = mapped.status;
    if (mapped.code) err.code = mapped.code;
    throw err;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const hasPg = prefersDirectPostgres() || hasDatabase();
    ...
    if (hasPg) {
      ...
      return res.status(503).json({
        error: "首單包郵須使用 Postgres 訂單後端；請設定 DATABASE_URL。",
      });
    }
    ...
    if (backend === "postgres" || backend === "supabase" || (!backend && hasPg)) {
      ...
      return res.status(503).json({
        error:
          "Order backend not configured. Set DATABASE_URL (Postgres) or Airtable variables, or set ORDER_BACKEND=postgres|airtable.",
      });
    }
